...
lkvへの更新はBaseの後ろにあるOverlayへ追記されます。更新や削除が増えると古いレコードがファイル内に残り、Overlayのインデックスもメモリを消費します。

`compact()`は有効なKey/ValueでBaseを再構築し、Overlayと古い世代を削除してデータベースファイルを物理的に縮小します。

```rust
// コンパクション中に参照されるスナップショットがないことを確認する
drop(snapshot);
db.compact()?;
```

コンパクションは自動では実行されません。Overlayが設定されたメモリ上限を超えると、新しい`WriteTransaction`は`Error::MaintenanceRequired`を返します。

コンパクション中は退避用のBaseを同じファイルの末尾に構築するため、完了まで追加のディスク容量が必要です。クラッシュした場合も再オープン時には同期済みのBaseから復旧できますが、ファイルの縮小が完了していない場合があります。その場合は再度`compact()`を実行できます。
